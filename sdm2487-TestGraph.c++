...
TEST_F(ImplementationTest, DefaultConstructor) {
	ASSERT_EQ(0, g.g.size());
	ASSERT_EQ(0, g.v.size());
...
TYPED_TEST(InterfaceTest, AddVertexTest4) {
	typedef typename TestFixture::vertex_descriptor vertex_descriptor;
	vertex_descriptor p = add_vertex(this->g);
	EXPECT_EQ(8, p);
	p = add_vertex(this->g);
...
TYPED_TEST(InterfaceTest, NumEdges4) {
	...
	add_edge(this->vdA, this->vdB, this->g);
	EXPECT_EQ(13, num_edges(this->g));
	add_edge(this->vdF, this->vdG, this->g);
	EXPECT_EQ(14, num_edges(this->g));
}
...
TYPED_TEST(InterfaceTest, NumVertices5) {
	typedef typename TestFixture::vertices_size_type vertices_size_type;
	EXPECT_EQ(8, num_vertices(this->g));
	add_vertex(this->g);
	EXPECT_EQ(9, num_vertices(this->g));
	add_vertex(this->g);
	EXPECT_EQ(10, num_vertices(this->g));
	add_vertex(this->g);
	EXPECT_EQ(11, num_vertices(this->g));
}
...
TYPED_TEST(InterfaceTest, Vertex4) {
	typedef typename TestFixture::vertex_descriptor vertex_descriptor;
	vertex_descriptor vd = vertex(3, this->g);
	EXPECT_EQ(this->vdB, vd);
}

TYPED_TEST(InterfaceTest, Vertex5) {
	typedef typename TestFixture::vertex_descriptor vertex_descriptor;
	vertex_descriptor vd = vertex(4, this->g);
	EXPECT_EQ(this->vdB, vd);
}

TYPED_TEST(InterfaceTest, Vertex6) {
	typedef typename TestFixture::vertex_descriptor vertex_descriptor;
