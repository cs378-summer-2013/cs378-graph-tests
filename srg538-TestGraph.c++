...
class OurTests : public testing::Test {
    ...
    virtual void add_edge_1() {
        add_edge(2, 1, _g);
    }

    virtual void add_edge_2() {
        add_edge(7, 0, _g);
    }

    virtual void add_edge_3() {
        add_edge(1, 0, _g);
    }

    virtual void add_edge_4() {
        add_edge(7, 2, _g);
    }

    virtual void add_edge_5() {
        add_edge(3, 6, _g);
    }
    virtual void add_edge_6() {
        add_edge(4, 6, _g);
    }
    virtual void add_edge_7() {
        add_edge(1, 2, _g);
        add_edge(4, 5, _g);
    }
};

TEST_F (OurTests, testHasCycle0){
    setupAcyclic();
    EXPECT_TRUE(!has_cycle(_g));}

TEST_F (OurTests, testHasCycle1){
    setupAcyclic();
    add_edge_1();
    EXPECT_TRUE(!has_cycle(_g));}

TEST_F (OurTests, testHasCycle2){
    setupAcyclic();
    add_edge_2();
    EXPECT_TRUE(has_cycle(_g));}

TEST_F (OurTests, testHasCycle3){
    setupAcyclic();
    add_edge_3();
    EXPECT_TRUE(has_cycle(_g));}

TEST_F (OurTests, testHasCycle4){
    setupAcyclic();
    add_edge_4();
    EXPECT_TRUE(has_cycle(_g));}

TEST_F (OurTests, testTopSort0){
    setupCyclic();
    try {
...
TEST_F (OurTests, testTopSort1){
    setupAcyclic();
    try {
        std::ostringstream out;
        topological_sort(_g, std::ostream_iterator<vertex_descriptor>(out, " "));
        EXPECT_TRUE(out.str() == "6 0 2 1 3 5 7 4 ");
    } catch (std::exception& e) {
        EXPECT_TRUE(false);
    }
}

TEST_F (OurTests, testTopSort2){
    setupAcyclic();
    try {
        std::ostringstream out;
        add_edge_5();
        topological_sort(_g, std::ostream_iterator<vertex_descriptor>(out, " "));
        EXPECT_TRUE(out.str() == "0 2 1 3 6 5 7 4 ");
    } catch (std::exception& e) {
        EXPECT_TRUE(false);
    }
}

TEST_F (OurTests, testTopSort3){
    setupAcyclic();
    try {
        std::ostringstream out;
        add_edge_6();
        add_edge_5();
        topological_sort(_g, std::ostream_iterator<vertex_descriptor>(out, " "));
        EXPECT_TRUE(out.str() == "0 2 1 3 5 4 6 7 ");
    } catch (std::exception& e) {
        EXPECT_TRUE(false);
    }
}

TEST_F (OurTests, testTopSort4){
    setupAcyclic();
    try {
        std::ostringstream out;
        add_edge_5();
        ...
        EXPECT_TRUE(out.str() == "0 1 2 3 4 6 5 7 ");
    } catch (std::exception& e) {
        EXPECT_TRUE(false);
    }
}
